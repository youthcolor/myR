...
library(rpart)
library(C50)
model = train(label~.,data = train,method = "rpart",preProcess="scale",
              trControl = control)
importance = varImp(model,scale = FALSE)
plot(importance)
head(importance$importance,25)
X <- rownames(head(importance$importance,25))

train.h2o <- as.h2o(train)

aml <- h2o.automl(y = "label", training_frame = train.h2o, 
                  max_runtime_secs = 600,
                  exclude_algos=c("GLM", "DeepLearning","DRF"),
                  sort_metric="logloss")
aml1 <- h2o.getModel("GBM_grid_0_AutoML_20180821_163542_model_5")
X <- head(aml1@model$variable_importances,20)$variable
h2o.removeAll()
...
write.csv(result,"pred_20180823_1100.csv",row.names = F,quote = F)
